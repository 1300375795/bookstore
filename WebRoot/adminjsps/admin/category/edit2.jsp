...
<body>
	...
	<form action="<c:url value='/admin/AdminCategoryServlet'/>"
		...
		<input type="hidden" name="method" value="editChild" /> <input
			...
			<!-- 注意select的value的值的给出需要到相应的option中给出而不是在select中 -->
				<option value="">===选择1级分类===</option>
				<c:forEach items="${parents }" var="parent">
					<option value="${parent.cid }"
					<c:if test="${child.parent.cid eq parent.cid }" >selected="selected"</c:if>>${parent.cname }</option>
				</c:forEach>
		</select><br /> 
		分类描述：
		<textarea rows="5" cols="50" name="desc" id="desc">${child.desc }</textarea>
		<br /> <input type="submit" value="修改二级分类" /> <input type="button"
